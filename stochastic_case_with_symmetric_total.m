...
i =1;
while i <= come
    A = [A,binornd(1,Lambda,T,1)];%node service request distribution
    i=i+1;
end
...
while b<=10 %γ
    ...
    while a<=10 %β
        ...
        while t<=T  %num of time t
            ...
            i =1;
            while i <= come
                if A(t,i)==1%node service request is TRUE
                    [M,I] = min(opt.Q);
                    B=[I;M];
        ...
        x = [x;a] ;
        y= [y;1-((((b*(1/15)/sevices)*opt.total.Q.L)/T)+a*opt.bignumaoi)/((((b*(1/15)/sevices)*total.Q.L)/T)+a*bignumaoi)]; %POA
...
hXLabel = xlabel('PoIs','Rotation',18,'Position',[5 6.5 5],'FontSize',15);
%hYLabel = ylabel('γ','Rotation',-40,'Position',[-1 2 0]);
hZLabel = zlabel('Times','Rotation',-90,'Position',[-1.5 0 800],'FontSize',15);
...
hold on;
% Add title and tags
%title('Symmetric case of PoA in terms of average total age.');
